      ...
      <div ref={scrollRef} className="flex-1 space-y-3 overflow-y-auto p-4">
        {messages.length === 0 && (
          <div className="flex flex-col items-center gap-2 py-12 text-center text-muted-foreground">
            <Bot className={cn("text-primary/40", embedded ? "h-12 w-12" : "h-10 w-10")} />
            <p className={cn("font-medium", embedded ? "text-base" : "text-sm")}>How can I help?</p>
            <p className={cn(embedded ? "text-sm" : "text-xs")}>Ask about inventory, sites, maintenance, or quotations.</p>
          </div>
        )}
        {messages.map((msg, i) => (
    ...
    return (
      <div className="flex h-full flex-col">
        <div className="flex items-center gap-2 border-b border-border bg-primary px-4 py-3 text-primary-foreground">
          ...
          <span className="text-sm font-semibold">OTNOAI Assistant</span>
        </div>
        {chatContent}
      </div>
    );
