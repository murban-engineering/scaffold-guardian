...
const HireQuotationWorkflow = ({
  ...
  const handleSelectPreviousClient = (quotation: HireQuotation) => {
    ...
    // When promoting a test quotation to a new HSQ, do NOT carry over the test client ID.
    // A fresh client ID will be generated by the DB sequence when the quotation is saved.
    const isPromotingTest = !isTestQuotation && !!quotation.quotation_number?.startsWith("TST-");
    const derivedClientId = isPromotingTest ? "" : (quotation.client_id || "");
    ...
    setHeader((prev) => ({
      ...
      // When promoting from test → HSQ, clear clientId so a fresh one is generated on save
      clientId: derivedClientId || (isPromotingTest ? "" : prev.clientId),
