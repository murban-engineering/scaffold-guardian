...
-- Fix both upsert_scaffold overloads so that adding stock to existing items
-- also increments qty_at_start, keeping the invariant: qty_at_start = available + on_hire
...
BEGIN
  ...
  IF existing_id IS NOT NULL THEN
    -- Adding more stock: increment both quantity (available) AND qty_at_start
    UPDATE scaffolds
    SET quantity     = COALESCE(quantity, 0) + p_quantity,
        qty_at_start = COALESCE(qty_at_start, 0) + p_quantity,
        updated_at   = now()
    WHERE id = existing_id
    RETURNING id INTO result_id;
  ELSE
    -- New item: qty_at_start = quantity
